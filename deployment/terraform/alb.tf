...
################################
locals {
  ecs_alb_name              = format("%s-%s-alb", var.ecs_service_name, var.env)
  ecs_alb_listen_https_port = var.alb_listen_port
}

locals {
  ecs_alb_tg_name     = format("%s-%s-tg", var.ecs_service_name, var.env)
  ecs_alb_tg_port     = var.task_service_port
  ecs_alb_tg_protocol = "HTTP"

  ecs_alb_sg_name = format("%s-%s-alb-sg", var.ecs_service_name, var.env)
}
