...
################################
locals {
  ecs_task_definition_name = "${var.ecs_service_name}-${var.env}"
  ecs_service_name         = "${var.ecs_service_name}-${var.env}-${random_id.ecs_service_random.hex}"
}

locals {
  task_container_name  = var.ecs_service_name
  task_container_image = "${aws_ecr_repository.demo.repository_url}:${var.ecr_image_tag}"
}
